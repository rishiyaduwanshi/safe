      ...
      <LoadingAnimation>
        <div className="max-w-7xl mx-auto">
              ...
              <div className="flex items-center gap-6 flex-wrap">
                  ...
                  <div className="flex gap-4 flex-wrap">
                    ...
                    {savedProfile?.state && (
                      <div className="px-3 py-2 rounded-lg text-sm font-semibold" style={{ background: '#6366F120', color: '#6366F1' }}>
                        📍 {savedProfile.state}, India
                      </div>
                    )}
                    {!savedProfile && (
                      <div className="px-3 py-2 rounded-lg text-sm font-semibold" style={{ background: '#F59E0B20', color: '#F59E0B' }}>
                        ⚠️ Verify your DL to submit reports
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="text-center min-w-50">
                  ...
                  <div className="mt-2 text-sm font-medium" style={{ color: '#10B981' }}>
                    ↗️ +{SAFETY_DATA.improvementFromLastMonth} this month
                  </div>
                </div>
              </div>
            </Card>
          ...
          <div className="flex mb-6 rounded-xl bg-background-secondary p-2 flex-wrap gap-2">
            {[{ id: 'violations', label: '⚠️ Violations' }, { id: 'personal', label: '👤 Personal Info' }].map((tab) => (
              <button
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === 'violations' && (
                <Card variant="glass" size="full">
                  <h3 className="text-xl font-bold text-white mb-6">⚠️ Violation History</h3>
                    ...
                    <div className="grid grid-cols-[repeat(auto-fit,minmax(400px,1fr))] gap-6">
                      <Card variant="glass" size="full">
                        <div className="flex justify-between items-center mb-4">
                          ...
                          <button onClick={handleRefetch} className="text-xs text-slate-400 hover:text-white px-2 py-1 rounded border border-slate-600 transition-colors">
                            🔄 Re-fetch
                          </button>
                        </div>
                        <div className="flex flex-col gap-4">
                          {[
                            { label: 'Full Name', value: savedProfile.name, icon: '👤' },
                            { label: 'Driver ID', value: savedProfile.driverId, icon: '🆔' },
                            { label: 'Phone', value: savedProfile.phone, icon: '📞' },
                            { label: 'Email', value: savedProfile.email, icon: '📧' },
                            { label: 'Address', value: savedProfile.address, icon: '📍' },
                            { label: 'Emergency Contact', value: `${savedProfile.emergencyContact.name} — ${savedProfile.emergencyContact.phone}`, icon: '🚨' },
                      ...
                      <Card variant="glass" size="full">
                        ...
                        <div className="flex flex-col gap-4">
                          {[
                            { label: 'License Number', value: savedProfile.licenseNumber, icon: '📋' },
                            { label: 'Vehicle Type', value: savedProfile.vehicleType, icon: '🏍️' },
                            { label: 'Issue Date', value: savedProfile.issueDate, icon: '📅' },
                            { label: 'Expiry Date', value: savedProfile.expiryDate, icon: '⏰' },
                          ].map((item) => <InfoRow key={item.label} {...item} />)}
                        </div>
                        <div
                        ...
                        >
                          <div className="flex items-center gap-2 mb-2">
                            <div className="text-2xl">{savedProfile.status === 'Valid' ? '✅' : '❌'}</div>
                  ...
                  {previewData && (
                    <Card variant="glass" size="full">
                      <div className="flex justify-between items-center mb-2">
                        <h3 className="text-xl font-bold text-white">🔍 Preview — confirm before saving</h3>
                        <button onClick={handleRefetch} className="text-xs text-slate-400 hover:text-white px-2 py-1 rounded border border-slate-600 transition-colors">
                          ✖ Cancel
                        </button>
                      </div>
                      ...
                      <div className="grid grid-cols-[repeat(auto-fit,minmax(330px,1fr))] gap-4 mb-6">
                        {[
                          { label: 'Full Name', value: previewData.name, icon: '👤' },
                          { label: 'Driver ID', value: previewData.driverId, icon: '🆔' },
                          { label: 'License Number', value: previewData.licenseNumber, icon: '📋' },
                          { label: 'Vehicle Type', value: previewData.vehicleType, icon: '🏍️' },
                          { label: 'Phone', value: previewData.phone, icon: '📞' },
                          { label: 'Address', value: previewData.address, icon: '📍' },
                          { label: 'Issue Date', value: previewData.issueDate, icon: '📅' },
                          { label: 'Expiry Date', value: previewData.expiryDate, icon: '⏰' },
                          { label: 'Emergency Contact', value: `${previewData.emergencyContact.name} — ${previewData.emergencyContact.phone}`, icon: '🚨' },
                        ].map((item) => <InfoRow key={item.label} {...item} />)}
                      </div>
                      {saveMutation.isError && (
                        <div className="p-3 rounded-lg text-sm mb-4" style={{ background: '#EF444420', color: '#EF4444', border: '1px solid #EF444440' }}>
                          ⚠️ {saveMutation.error?.message ?? 'Failed to save. Please try again.'}
                        </div>
                      )}
                      <button
                        ...
                        <form onSubmit={handleLookup} className="flex flex-col gap-4 text-left">
                          ...
                          {(dlError || lookupMutation.isError) && (
                            <div className="p-3 rounded-lg text-sm" style={{ background: '#EF444420', color: '#EF4444', border: '1px solid #EF444440' }}>
                              ⚠️ {dlError ?? lookupMutation.error?.message ?? 'Something went wrong'}
                            </div>
                          )}
                          <button
...
// const SAFETYDATA = { currentScore: 850, maxScore: 1000, improvementFromLastMonth: 15 };

// // Load cached license data from localStorage on mount
// useEffect(() => {
//   const cached = localStorage.getItem(CACHE_KEY);
//   if (cached) {
//     try { setLicenseData(JSON.parse(cached)); } catch { /* ignore */ }
//   }
...
//   return () => { clearTimeout(timer); clearTimeout(scoreTimer); };
// }, [CACHE_KEY]);
...
//   if (!DL_REGEX.test(trimmed)) {
//     setFetchError('Invalid DL format. Expected format: HR-0619850123456 (State code + RTO code + Year + Serial)');
//     return;
//   }
...
//   setFetchError(null);
//   try {
//     // Name is automatically picked from your account — no need to pass it
//     const res = await licenseApi.lookup({ dlNumber: trimmed });
//     const data = res.data;
//     localStorage.setItem(CACHE_KEY, JSON.stringify(data));
//     setLicenseData(data);
//   } catch (err) {
//     setFetchError(err?.message ?? 'Failed to fetch license data. Please try again.');
//   } finally {
//     setIsFetching(false);
//   }
// };

// const displayName = licenseData?.name ?? user?.name ?? 'Driver';
...
//                 <div
//                   className="absolute -bottom-1 -right-1 text-white px-2 py-1 rounded-full text-sm font-bold border-2"
//                   style={{ background: getScoreGrade(SAFETYDATA.currentScore).color, borderColor: '#1e293b' }}
//                 >
...
//                 <p className="text-lg text-slate-300 mb-4">
//                   {licenseData
//                     ? `License: ${licenseData.licenseNumber} • ${licenseData.vehicleType}`
//                     : user?.email ?? ''}
//                 </p>
//                 <div className="flex gap-4 flex-wrap">
//                   {licenseData && (
...
//                   <p className="text-white font-semibold text-lg mb-1">No violations recorded</p>
//                   <p className="text-slate-400 text-sm">
//                     Violation history will be available once the traffic authority integration is live.
//                   </p>
//                 </div>
//               </Card>
//             )}
...
//                   <div className="max-w-lg mx-auto py-8 text-center">
//                     <div className="text-5xl mb-4">🏛️</div>
//                     <h3 className="text-2xl font-bold text-white mb-2">Fetch License Details</h3>
//                     <p className="text-slate-400 mb-2 text-base">
//                       Enter your Driving License number to fetch your details from the
//                       <span className="text-indigo-400 font-medium"> Sarathi Parivahan database</span>.
//                     </p>
//                     <p className="text-slate-500 text-sm mb-8">
//                       Your account name <span className="text-slate-300 font-medium">({user?.name})</span> will be used automatically for verification.
//                     </p>
//                     <form onSubmit={handleLicenseLookup} className="flex flex-col gap-4 text-left">
//                       <div>
...
//                           type="text"
//                           value={dlInput}
//                           onChange={(e) => { setDlInput(e.target.value); setFetchError(null); }}
//                           placeholder="e.g. HR0620110012345 or HR-06-2011-0012345"
//                           className="w-full px-4 py-3 rounded-lg text-white text-base outline-none focus:ring-2 focus:ring-indigo-500"
//                           style={{ background: '#1e293b', border: '1px solid #334155' }}
//                           disabled={isFetching}
...
//                       <button
//                         type="submit"
//                         disabled={isFetching || !dlInput.trim()}
//                         className="w-full py-3 rounded-lg font-bold text-white text-base transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
//                         style={{ background: 'linear-gradient(to right, #6366F1, #8B5CF6, #06B6D4)' }}
//                       >
//                         {isFetching ? (
...
//                             Fetching from Sarathi Parivahan...
//                           </span>
//                         ) : '🔍 Fetch License Details'}
//                       </button>
//                     </form>
...
//                       <h3 className="text-xl font-bold text-white">👤 Personal Information</h3>
//                       <button
//                         onClick={() => { setLicenseData(null); localStorage.removeItem(CACHE_KEY); setDlInput(''); }}
//                         className="text-xs text-slate-400 hover:text-white px-2 py-1 rounded border border-slate-600 transition-colors"
//                       >
//                         🔄 Re-fetch
//                       </button>
//                     </div>
//                     <div className="flex flex-col gap-4">
//                       {[
//                         { label: 'Full Name', value: licenseData.name, icon: '👤' },
//                         { label: 'Driver ID', value: licenseData.driverId, icon: '🆔' },
//                         { label: 'Phone', value: licenseData.phone, icon: '📞' },
//                         { label: 'Email', value: licenseData.email, icon: '📧' },
//                         { label: 'Address', value: licenseData.address, icon: '📍' },
//                         { label: 'Emergency Contact', value: `${licenseData.emergencyContact.name} - ${licenseData.emergencyContact.phone}`, icon: '🚨' },
//                       ].map((item) => (
//                         <div key={item.label} className="flex items-center gap-3 p-3 rounded-lg border" style={{ background: '#1e293b', borderColor: '#334155' }}>
...
//                     <div className="flex flex-col gap-4">
//                       {[
//                         { label: 'License Number', value: licenseData.licenseNumber, icon: '📋' },
//                         { label: 'Vehicle Type', value: licenseData.vehicleType, icon: '🏍️' },
//                         { label: 'Issue Date', value: licenseData.issueDate, icon: '📅' },
//                         { label: 'Expiry Date', value: licenseData.expiryDate, icon: '⏰' },
//                       ].map((item) => (
//                         <div key={item.label} className="flex items-center gap-3 p-3 rounded-lg border" style={{ background: '#1e293b', borderColor: '#334155' }}>
//                           <div className="text-2xl">{item.icon}</div>
//                           <div className="flex-1">
...
//                       className="mt-6 p-4 rounded-lg border"
//                       style={{
//                         background: licenseData.status === 'Valid' ? '#10B98110' : '#EF444410',
//                         borderColor: licenseData.status === 'Valid' ? '#10B98130' : '#EF444430',
//                       }}
//                     >
//                       <div className="flex items-center gap-2 mb-2">
//                         <div className="text-2xl">{licenseData.status === 'Valid' ? '✅' : '❌'}</div>
//                         <div className="font-bold text-base" style={{ color: licenseData.status === 'Valid' ? '#10B981' : '#EF4444' }}>
//                           License Status: {licenseData.status}
//                         </div>
//                       </div>
//                       <p className="text-slate-400 text-sm">
//                         {licenseData.status === 'Valid'
//                           ? `Your driving license is currently valid and will expire on ${licenseData.expiryDate}. You'll receive a reminder 60 days before expiry.`
//                           : `Your driving license expired on ${licenseData.expiryDate}. Please renew it at your nearest RTO.`}
//                       </p>
//                     </div>
//                   </Card>
//                 </div>
